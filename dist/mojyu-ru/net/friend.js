 ...
 * 1. 申請を送る
 */
export async function sendRequest(targetUuid, myPublicKey) {
 ...
 * 3. 申請を承認する（相手側の操作）
 */
export async function approveRequest(recordUuid, myPublicKey) {
    return await supabase
        .from('Friendsinsei')
        .update({
 ...
 * 4. お掃除（削除）
 */
export async function deleteRequest(recordUuid) {
    return await supabase
        .from('Friendsinsei')
        .delete()
        .eq('uuid', recordUuid);
