...
btnroom.onclick = () => {
    ...
    wss.onmessage = async (event) => {
        ...
        if (data.type === "join-broadcast" && data.name !== myName) {
            ...
            if (!aeskey)
                return;
            const decrypted = await decrypt(aeskey, base64ToUint8Array(data.iv), base64ToUint8Array(data.data).buffer);
