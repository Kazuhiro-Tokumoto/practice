...
async function main() {
    ...
    // --- 3. 鍵関連の変数宣言 ---
    let myKeys: any = null;           // 自分の鍵ペア
    let aeskey: CryptoKey | null = null;
    let peerEdPubKey: CryptoKey | null = null; // 相手の署名検証用
    let room: string;
    ...
    pininput.type = "password"; pininput.placeholder = "PIN(数字)"; pininput.inputMode = "numeric";
    ...
    async function emergencyWipe() {
        if (!confirm("データをすべて破棄しますか？")) return;
    ...
    async function restoreKey(pin: string) {
        ...
        if (!dbData || dbData.salt === null) {
            ...
            const aesKey = await deriveKeyFromPin(pin, salt);
            const seed = new Uint8Array(await crypto.subtle.decrypt({ name: "AES-GCM", iv: iv.buffer as ArrayBuffer }, aesKey, encryptedSeed.buffer as ArrayBuffer));
    ...
    async function ed25519Handler(mode: number, key: CryptoKey, dataB64: string, sigB64 = "") {
        ...
        if (mode === 1) {
            ...
            return await window.crypto.subtle.verify({ name: "Ed25519" }, key, sigUint8 as BufferSource, dataUint8);
        }
    }

    async function testPublicKeyFetch(targetUuid: string) {
    ...
    wss.onmessage = async (event: MessageEvent) => {
        ...
        if (data.type === "DH" && data.name !== name) {
            ...
            if (peerData && peerData.x25519_pub) {
                ...
                const theirXKey = await window.crypto.subtle.importKey("raw", peerXRaw as BufferSource, { name: "X25519" }, true, []);
                aeskey = await deriveSharedKey(myKeys.xPriv, theirXKey);
                
                const peerEdRaw = await base64ToUint8Array(peerData.ed25519_pub);
                peerEdPubKey = await window.crypto.subtle.importKey("raw", peerEdRaw as BufferSource, { name: "Ed25519" }, true, ["verify"]);
                console.log("✨ 鍵合体完了");
            }
        } else if (data.type === "message" && data.name !== name) {
            try {
                if (!aeskey || !peerEdPubKey) return;
                ...
                if (!isValid) return console.error("署名検証失敗");
                const decrypted = await decrypt(aeskey, iv, encrypted.buffer as ArrayBuffer);
    ...
    btnroom.addEventListener("click", () => {
        ...
        roomSelection.style.display = "none"; chatContainer.style.display = "flex";
        wss.send(JSON.stringify({ type: "join", room, name, uuid: storedUuid, token: storedToken }));
    });

    sendBtn.onclick = () => { if (input.value && aeskey) sendEncryptedMessage(input.value, aeskey); input.value = ""; };
    ...
    if (storedPin === null) {
        ...
        pinbtn.onclick = async () => {
            ...
            pinContainer.style.display = "none"; roomSelection.style.display = "flex";
        };
    } else {
