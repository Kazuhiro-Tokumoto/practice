...
const SUPABASE_URL: string = 'https://cedpfdoanarzyxcroymc.supabase.co';
const SUPABASE_KEY: string = 'sb_publishable_E5jwgv5t2ONFKg3yFENQmw_lVUSFn4i';

const supabaseClient = (window as any).supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
...
async function init(): Promise<void> {
    ...
    const authUi = document.getElementById('auth-ui') as HTMLDivElement | null;
    ...
    if (session) {
        ...
        const fullName: string = user.user_metadata.full_name ?? "不明なユーザー";
